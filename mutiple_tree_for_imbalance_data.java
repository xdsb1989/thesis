/*
 * This is the program for decision tree. 
 * Using MDL algorithem to do the post pruning.
 */
...
public class mutiple_tree_for_imbalance_data {
		...
		*/
		...
		for (int i=0;i<examples;i++)
			if (!original_train[i][original_train[i].length-1].equals("No_action"))
				number_of_actions++;
		int number_of_trees;//how many trees to build
		...
		for (int ith_tree = 0;ith_tree<number_of_trees;ith_tree++){//from this part, begins to build mutiple trees
			...
			System.out.println("train number: "+training_number);
			...
			//for (int i=0;i<continuous_attribute_list.size();i++)
			//	System.out.println(continuous_attribute_list.get(i).name+" "+continuous_attribute_list.get(i).thresholds);
			...
			Build_tree(root[ith_tree],attribute_list,train,training_number,
					...
					continuous_attribute_index,continuous_attribute_list,"",1);
		...
		double confidence[] = new double[test_number];
		...
		for (int j=0;j<result.length;j++)
			System.out.printf("%s   \t%s    \t%s   \t%f\n",before_processing[j],result[j],test[j][test[j].length-1],confidence[j]);
	...
	private static void auto_analyze_sequence(String[] result, String[][] original_train, 
				String[] before_processing, String [][]test,double []confidence) {
		int drink_min = 10;
		int drink_max = 0;
		int slap_min = 10;
		int slap_max = 0;
	...
	private static void Trees_prediction(Tree_node[] root, String[][] test,
			String[] result, String[] Class_type, ArrayList<Attribute_list> attribute_list, double []confidence) {
