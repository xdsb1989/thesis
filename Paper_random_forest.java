...
public class Paper_random_forest {
	...
	public static void main(String args[]) throws IOException{
		...
		*/
		...
		for (int ith_tree = 0;ith_tree<number_of_trees;ith_tree++){//from this part, begins to build multiple trees
			...
			int number_of_random_attribute =  (int) (Math.log(number_of_concreat_attribute + continuous_attribute_list.size()) / Math.log(2)) + 1;
			double rate_of_attribute_choose = (double) number_of_random_attribute / (number_of_concreat_attribute + continuous_attribute_list.size());
			System.out.println("number_of_random_attribute for each node: "+number_of_random_attribute);
			...
			Build_tree(root[ith_tree],attribute_list,train,training_number,
					attribute_list.size()-1, continuous_attribute_number,number_of_random_attribute,Class_type,
					continuous_attribute_list,"",1,rate_of_attribute_choose);
	...
	private static void Build_tree(Tree_node root, ArrayList<Attribute_list> attribute_list, String[][] train,
			int examples, int attribute_number, int continuous_attribute_number, int current_attribute_number,
			String[] class_type, ArrayList<Continuous_Attribute_list> continuous_attribute_list, 
			String default_type,int current_level, double rate_of_attribute_choose) {
		
		
		int attribute_index [] = new int [attribute_number];
		int continuous_attribute_index [] = new int [continuous_attribute_number];
		int number_of_get_attribute = 0;
		...
		for (int i=0;i<attribute_number;i++){
			if (attribute_list.get(i).continuous == 0){
				...
				if (rd.nextDouble() < rate_of_attribute_choose){
					attribute_index[i] = 1;
					number_of_get_attribute++;
					if (number_of_get_attribute == current_attribute_number)
						break;
				}
			}
		}
		...
		if (temp_1>temp_2){
			...
			for (int i=1;i <= root.range;i++){//this loop is for each value of the attribute,and extend it to be sub-tree
				...
				Build_tree(root.value_range[i],attribute_list,temp_examples,number,attribute_number,continuous_attribute_number,
						current_attribute_number,class_type,continuous_attribute_list, 
						default_type,current_level,rate_of_attribute_choose);
			}
		}
		if (temp_1<=temp_2){
			//current_attribute_number = current_attribute_number - 1;//reduce the available attributes
			...
			Build_tree(root.small,attribute_list,small_examples,small_number,attribute_number,continuous_attribute_number,
					current_attribute_number,class_type,continuous_attribute_list, default_type,current_level,rate_of_attribute_choose);
